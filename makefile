# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    Makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: dagarcia <marvin@42.fr>                    +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2021/02/04 12:22:47 by dagarcia          #+#    #+#              #
#    Updated: 2021/02/19 12:56:44 by pperez-m         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

CC = GCC
FLAGS = -Wall -Wextra -Werror
RM = rm -f
AR = ar -cr
NAME = libft.a
SOURCE = ft_memset.c ft_bzero.c ft_memcpy.c ft_memccpy.c ft_memmove.c \
	ft_memchr.c ft_memcmp.c ft_strlen.c ft_strlcpy.c ft_strlcat.c \
	ft_strchr.c ft_strrchr.c ft_strnstr.c ft_strncmp.c ft_atoi.c \
	ft_isalpha.c ft_isdigit.c ft_isalnum.c ft_isascii.c ft_isprint.c \
	ft_toupper.c ft_tolower.c ft_substr.c ft_strjoin.c ft_strtrim.c \
	ft_putchar_fd.c ft_putstr_fd.c ft_split.c ft_itoa.c \
	ft_putendl_fd.c ft_putnbr_fd.c ft_calloc.c ft_strdup.c
BONUS = ft_lstnew.c ft_lstadd_front.c ft_lstsize.c ft_lstlast.c \
	ft_lstadd_back.c ft_lstdelone.c ft_lstclear.c ft_lstiter.c ft_lstmap.c
OBJ_SRC =$(SOURCE:.c=.o)
OBJ_BNS = $(BONUS:.c=.o)
LIBS = libft.h

all: $(NAME)

$(NAME): $(OBJ_SRC)
	$(AR) $(NAME) $(OBJ_SRC)

so:
	$(CC) -fPIC $(CFLAGS) $(SRC)
	gcc -shared -o libft.so $(OBJ)

clean:
	$(RM) $(OBJ_SRC) $(OBJ_BNS)

fclean: clean
	$(RM) $(NAME)

bonus: $(OBJ_SRC) $(OBJ_BNS)
	$(AR) $(NAME) $(OBJ_SRC) $(OBJ_BNS)

norm: norminette *.c *.h

re: fclean all

.PHONY: bonus all clean fclean re


